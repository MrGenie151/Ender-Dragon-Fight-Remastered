...
execute in minecraft:the_end as @e[type=ender_dragon,tag=!engaged] run data merge entity @s {Health:9999f,attributes:[{id:"minecraft:max_health",base:9999}]}
execute in minecraft:the_end as @e[type=ender_dragon,tag=!engaged] store result entity @s attributes[{id:"minecraft:max_health"}].base double 1.0 run scoreboard players get DragonHealth EDFR.config
